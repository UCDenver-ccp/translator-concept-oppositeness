...
for (file_number in 1:length(files)) {
  if (1) { print(paste("File:", files[file_number])) }  
  sheet <- NULL
  ...
  if (1) { print("Add counts to data.frame")}
  sheet <- cbind(sheet,counts) 
  column.names <- c(column.names, "counts")
  if (1) { print("Reset column names") }
  colnames(sheet) <- column.names
  
  if (1) { print("Finally, generate the graph") }
