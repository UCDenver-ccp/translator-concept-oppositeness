...
while (my $line = <IN>) {
	# here we're assuming a single word only--TODO
        ...
        if ($line =~ /^id:\s+((MONDO|CHEBI|SO|BFO|RO|MP|SNOMED|PR|GO|CL|UBERON|NCBITaxon|PATO|IntAct:EBI-|MI|MOD|NCBIGene|OGMS|OMIM|PR|ReTO|SIO|UniProt|HP|SCTID)[:_]\d+)$/o) { 
          $id = $1; 
          $DEBUG && print "ID: <$id>\n";
        } # close if-ID
...
my %opposites = ("fast" => "slow",
	         "slow" => "fast",
                 ...
                 "exportation" => "importation"); # hyper and hypo are tough ones 'cause they're prefixes but my code for these requires word boundaries BUT these aren't negatives, per se.  Solution: a small function for this
